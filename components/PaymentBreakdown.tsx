...
import {
  ...
  PaymentStatus,
} from "../redux/types/types.ContactTransactionPair";
import { AppState } from "../redux/root.reducer";
import { Dispatch } from "redux";
import { Transaction } from "../redux/types/types.Transaction";
import {
  addMultipleContactTransactionPairs,
  editAmount,
  removeContactFromTransaction,
  removeContactsByTransactionId,
} from "../redux/contactTransactionPair/contactTransactionPair.action";
...
> {
  ...
  componentDidMount() {
    this.setState({
      contactTransactionPairs: lodash.cloneDeep(
        this.props.contactTransactionPairs
      ),
    });
  }
  ...
  updateSearch = (search: string) => {
    this.setState({ search }, () => {
      this.setState({
        searchResultList:
          this.state.search === "" ? [] : this.getContactByName(search),
      });
    });
  };
  ...
  getContactByName = (name: string) => {
    ...
    return this.props.contacts.filter((contact: Contact) => {
  ...
  removeContactFromTransaction = (contactId: string) => {
    let contactTransactionPairList = this.state.contactTransactionPairs;
    let indexToRemove = contactTransactionPairList.findIndex(
      (contactTransactionPair) => {
        return contactTransactionPair?.contactId === contactId;
      }
    );
  ...
  editAmount = (contactId: string, amount: string) => {
    ...
    let indexToModify = this.state.contactTransactionPairs.findIndex(
      (contactTransactionPair) => {
        return contactTransactionPair?.contactId === contactId;
      }
    );
  ...
  updateContactsByTransactions = () => {
    let changeHappened = !lodash.isEqual(
      this.props.contactTransactionPairs,
      this.state.contactTransactionPairs
    );
    if (changeHappened) {
      this.props.removeContactsByTransactionId(this.props.currentTransaction);
      this.props.addMultipleContactTransactionPairs(
        this.state.contactTransactionPairs
      );
    }
  };
  ...
  static getDerivedStateFromProps = (
    nextProps: PaymentBreakdownProps,
    prevState: PaymentBreakdownState
  ) => {
  ...
  getContactById = (contactId: string) => {
    return this.props.contacts.find((contact: Contact) => {
  ...
  render() {
      ...
      <FlatList
        ...
        renderItem={({ item: contact }) => {
          return (
            <ListItem
              onPress={() => {
                this.addContactToTransaction(contact);
                this.setState({ contactViewVisible: false });
              }}
              key={contact.id}
            >
    ...
    return (
      <View>
        ...
        <Overlay
          isVisible={this.state.contactViewVisible}
          onBackdropPress={() => this.toggleContactView()}
        ...
        <View>
          <ListItem
            ...
            containerStyle={
              this.state.editable
                ? { backgroundColor: "#000000", flex: 0.9 }
                : styles.displayHide
            }
          >
            <ListItem.Content>
              ...
              {this.state.contactTransactionPairs.map(
                (contactTransactionPair) => (
                  <View
                  ...
                  >
                    ...
                    >
                      <ListItem.Title style={{ fontSize: 14 }}>
                        {
                          this.getContactById(
                            contactTransactionPair!.contactId
                          )!.name
                        }
                      </ListItem.Title>
                    </ListItem>
...
const mapStateToProps = (
  state: AppState,
  ownProps: ParentComponentProps
): StateProps => {
  const contactTransactionPairs: ContactTransactionPair[] =
    state.contactTransactionPairReducer;
  const contacts: Contact[] = state.contactReducer;

  const currentTransactionId = ownProps.currentTransaction.id;
  let filteredContactTransactionPairs = contactTransactionPairs.filter(
    (contactTransactionPair) => {
...
const mapDispatchToProps = (dispatch: Dispatch): DispatchProps => ({
  addMultipleContactTransactionPairs: (
    contactTransactionPairs: ContactTransactionPair[]
  ) => dispatch(addMultipleContactTransactionPairs(contactTransactionPairs)),
  removeContactFromTransaction: (contactId: string) =>
    dispatch(removeContactFromTransaction(contactId)),
  removeContactsByTransactionId: (transactionId: string) =>
    dispatch(removeContactsByTransactionId(transactionId)),
  editAmount: (contactId: string, amount: number) =>
    dispatch(editAmount(contactId, amount)),
});
