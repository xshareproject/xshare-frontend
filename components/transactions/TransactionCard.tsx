...
function TransactionCard(props: TransactionCardProps) {
  ...
  return (
    <TouchableHighlight
      onPress={() => props.navigationCallback(props.transaction)}
    >
      <React.Fragment>
        <View
...
const mapStateToProps = (
  state: AppState,
  ownProps: TransactionCardProps
): StateProps => {
  let contactTransactionPair = state.contactTransactionPairReducer.filter(
    (contactTransactionPair: ContactTransactionPair) => {
      return contactTransactionPair.transactionId == ownProps.transaction.id;
    }
  );
  let transactionContacts = contactTransactionPair.map(
    (contactTransactionPair: ContactTransactionPair) => {
      return state.contactReducer.find(
        (value) => value.id === contactTransactionPair.contactId
      );
    }
  );
